...
type Model struct {
	...
	SettingsScreenIndex          int    // NEW: Renamed from StatsScreenIndex
	...
	// --- Auth / Link Flow State ---
	Linking       bool
	LinkCode      string
	LinkExpiresAt int64

	// --- Cached user info for Settings preview (to avoid blocking the UI) ---
	ClerkUserInfo           string
	IsFetchingClerkUserInfo bool
	ClerkUserInfoToken      string
	ClerkUserInfoAttempted  bool
}
...
// It includes data needed for history recording.
type CommandFinishedMsg struct {
	Err            error             // The error, if any, from command execution
	CommandName    string            // The name of the command that was run
	ProjectPath    string            // The project path where the command ran
	Placeholders   map[string]string // The placeholders/variables used
	GeneratedFiles []string          // Files generated by the command (if applicable)
}

// ClerkUserInfoMsg carries fetched user info for the Settings preview
type ClerkUserInfoMsg struct {
	Info  string
	Token string
}
